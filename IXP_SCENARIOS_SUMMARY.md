...
- Traffic balancing across multiple satellite-to-IXP links
- Link quality tracking and rebalancing
...
### Visible Scenario (Satellite-side Interface Flaps)

**Control-plane timeline:**
1. **t=0.1s:** Backup satellite links (1020002, 1030002, etc.) disabled → no immediate effect (primary links still up)
2. **t=50.0s:** Primary satellite links disabled (e.g., 1020001) → BGP session 102-110 drops at ~t=53s (STATE_CHANGE: ESTABLISHED→IDLE)
3. **t=55.0s:** Backup link restored (1020002) — but BGP session does NOT immediately re-open
4. **t=94.0s:** Delayed re-establishment: IDLE→OPEN_CONFIRM→ESTABLISHED (BGP retry backoff)
5. **t=120.0s:** Direct core link 101-102 disabled → 101-102 BGP session drops, no recovery observed in 260s window

**Data-plane observations:**
- Probe pairs with alternate paths (e.g., 102→104) recover after 41–44s outage
- Probe pairs dependent on 101-102 or 102-110 show no recovery (status: `no_recovery`)

### Hidden Scenario (VIXP-Managed Failover)

**Control-plane timeline:**
1. **t=0.1–50.0s:** VIXP manages satellite link quality; BGP sessions remain stable
2. **t=50.0s:** Second satellite link disabled → VIXP rebalances traffic, no immediate BGP drop
3. **t=53.0s:** 102-110 BGP session eventually drops (similar to visible scenario)
4. **t=94.0s:** Re-establishment (same delay as visible)
5. **t=120.0s:** Direct link 101-102 disabled → 101-102 drops, partial recovery observed

**Data-plane observations:**
- Some probe pairs recover after failover (status: `switched_and_recovered`)
- Others remain blocked due to topology constraints (status: `no_recovery`)
...
Generated by `bgp_ixp_scenario_runner.py` → `summary.md`:

```markdown
# BGP IXP Scenario 2: visible AS110 failover
...
| min | 4.005 |
| max | 46.007 |
| packet_loss_percent | 13.17 |

## Control Plane Windows

| failed_link | link_down_time_s | link_up_time_s | OPEN | UPDATE_announce | UPDATE_withdraw | STATE_CHANGE |
|---|---:|---:|---:|---:|---:|---:|
| AS102-AS110 | 0.1 | 55.0 | 42 | 257 | 3 | 125 |
| AS101-AS102 | 120.0 | | 0 | 8 | 2 | 24 |
```
