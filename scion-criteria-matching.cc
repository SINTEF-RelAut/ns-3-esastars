...
struct beacon {
  int64_t initiation_time, expiration_time, next_initiation_time, next_expiration_time, latency_stat, bwd_stat;
  path* the_path;
  std::string key;
...
main (int argc, char *argv[])
{
  ...
  while (curNode) {
    ...
    std::uniform_int_distribution<int> link_count_distribution(1, 50);
