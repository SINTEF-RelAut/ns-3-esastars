...
namespace ns3 {
    ...
    public:
        SCIONCapableNode(uint32_t system_id, uint16_t isd_number, uint16_t as_number, host_addr_t local_address,
            ...
            : Node(system_id), isd_number(isd_number), as_number(as_number), local_address(local_address),
              latitude(latitude), longitude(longitude), AS(AS) {
    ...
    protected:
        ...
        SCIONPacket *
        create_scion_packet(const Payload &payload, payload_type_t payload_type, ia_t dst_ia, host_addr_t dst_host,
