...
std::function<R(Types...)> bind_factory(R (U::*f)(Types...), U *val, std::integer_sequence<int, indices...> /*seq*/) {
    return std::bind(f, val, my_placeholder<indices + 1>::ph...);
}

template<class R, class... Types, class U>
std::function<R(Types...)> function_factory(R (U::*f)(Types...), U *val) {
...
namespace ns3 {
    ...
    public:
        UserDefinedEvents(YAML::Node &config, NodeContainer &AS_nodes, std::map<int32_t, uint16_t> &real_to_alias_as_no,
    ...
    private:
        ...
        void send_a_packet(std::string src_isd_number, std::string real_src_as_no, std::string src_local_address,
                           std::string dst_isd_number, std::string real_dst_as_no, std::string dst_local_address,
                           std::string pyload_size);

        void send_packet_batch(std::string src_isd_number, std::string real_src_as_no, std::string src_local_address,
                               std::string dst_isd_number, std::string real_dst_as_no, std::string dst_local_address,
