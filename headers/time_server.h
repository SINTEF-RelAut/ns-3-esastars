...
#define NTP_REQ_GLOBAL_SYNC_DIFF "2s"
    ...
    enum SNAPSHOT_TYPE { LOCAL_SNAPSHOT = 0, ASSERT_OFFSET_DIFF = 1, PRINT_OFFSET_DIFF = 2, SNAPSHOT_OFF = 3 };
    ...
    public:
        TimeServer(uint32_t system_id, uint16_t isd_number, uint16_t as_number, host_addr_t local_address,
                   ...
                   Time max_drift_per_day, bool jitter_in_drift, uint32_t max_drift_coefficient, Time global_cut_off,
            ...
            : SCIONHost(system_id, isd_number, as_number, local_address, latitude, longitude, AS),
              ...
              max_drift_coefficient(max_drift_coefficient), global_cut_off(global_cut_off), first_event(first_event),
              last_event(last_event), snapshot_period(snapshot_period),
              ...
              server_type(TIME_SERVER_TYPE_MAP[server_type]), snapshot_type(SNAPSHOT_TYPE_MAP[snapshot_type]),
              alg_v(ALG_V_MAP[alg_v]), minimum_malicious_offset(minimum_malicious_offset),
    ...
    private:
        ...
        std::map<std::string, TIME_SERVER_TYPE> TIME_SERVER_TYPE_MAP = {
                {"NORMAL", TIME_SERVER_TYPE::NORMAL},
