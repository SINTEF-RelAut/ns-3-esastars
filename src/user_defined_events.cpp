...
namespace ns3 {
    void UserDefinedEvents::run_user_specified_event(const std::string& func_name, std::vector<std::string> vec) {
        switch (vec.size()) {
            case 0:
    ...
    void UserDefinedEvents::construct_func_map () {
        ...
        function_name_to_function["send_packet"] = function_factory(&UserDefinedEvents::send_a_packet, this);
        function_name_to_function["send_packet_batch"] = function_factory(&UserDefinedEvents::send_packet_batch, this);
        function_name_to_function["time_references_down"] = function_factory(&UserDefinedEvents::time_references_down, this);
        function_name_to_function["time_references_up"] = function_factory(&UserDefinedEvents::time_references_up, this);
    }

    void UserDefinedEvents::read_and_schedule_user_defined_events(const std::string& events_file_str) {
        ...
        for(auto const & event : events_json.at("events")) {
            ...
            Simulator::Schedule(time, &UserDefinedEvents::run_user_specified_event, this, func_name, args_v);
        }
    }

    void UserDefinedEvents::add_a_host (std::string isd_number, std::string real_as_no, std::string local_address){}

    void UserDefinedEvents::link_down (std::string isd_number, std::string real_as_no, std::string if_id){}
    void UserDefinedEvents::link_up (std::string isd_number, std::string real_as_no, std::string if_id){}

    void UserDefinedEvents::send_a_packet (std::string src_isd_number, std::string real_src_as_no, std::string src_local_address,
                                           std::string dst_isd_number, std::string real_dst_as_no, std::string dst_local_address,
                                           std::string pyload_size){}

    void UserDefinedEvents::send_packet_batch (std::string src_isd_number, std::string real_src_as_no, std::string src_local_address,
                                               std::string dst_isd_number, std::string real_dst_as_no, std::string dst_local_address,
                                               std::string pyload_size, std::string no_pkts){}

    void UserDefinedEvents::time_references_down ()
    {
    ...
    void UserDefinedEvents::time_references_up ()
    {
        for (uint32_t i = 0; i < AS_nodes.GetN(); ++i) {
            SCION_AS* scion_as = dynamic_cast<SCION_AS*>(PeekPointer(AS_nodes.Get(i)));
            TimeServer* time_server = dynamic_cast<TimeServer*>(scion_as->GetHost(2));
