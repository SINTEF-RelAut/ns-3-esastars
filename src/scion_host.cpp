...
void
SCIONHost::receive_registered_path_segments (path_segment_type seg_type, ia_t src_ia, ia_t dst_ia,
...
void
SCIONHost::receive_cached_path_segments (path_segment_type seg_type, ia_t src_ia, ia_t dst_ia,
                                         cached_path_segs_per_dst_t *path_seg)
{
}

void
SCIONHost::remove_expired_segments ()
{
}

void
SCIONHost::request_for_path_segments (ia_t dst_ia)
{
...
void
SCIONHost::cache_path_segment (path_segment_type seg_type, ia_t src_ia, ia_t dst_ia,
...
void
SCIONHost::send_request_for_path_segments (path_segment_type seg_type, ia_t src_ia, ia_t dst_ia)
{
...
void
SCIONHost::search_in_cached_segments (ia_t dst_ia, std::vector<const PathSegment *> &the_path,
      ...
      for (auto const &[down_seg_src_ia, down_path_segs] : *cached_down_path_segments.at (dst_ia))
        {
          if (cached_core_path_segments.find (down_seg_src_ia) != cached_up_path_segments.end ())
            {
              the_path.push_back (cached_core_path_segments.at (down_seg_src_ia)
                                      ->begin ()
                                      ->second->begin ()
                                      ->second);
              ...
              for (auto const &[core_seg_src_ia, core_path_segs] :
                   *cached_core_path_segments.at (down_seg_src_ia))
                {
                  if (cached_up_path_segments.find (core_seg_src_ia) !=
                    ...
                    {
                      the_path.push_back (cached_up_path_segments.at (core_seg_src_ia)
                                              ->at (ia_addr)
                                              ->begin ()
                                              ->second);
                      the_path.push_back (core_path_segs->begin ()->second);
...
void
SCIONHost::process_received_packet (uint16_t local_if, SCIONPacket *packet, Time receive_time)
{
...
void
SCIONHost::modify_pkt_upon_send (SCIONPacket *packet)
{
}
} // namespace ns3
