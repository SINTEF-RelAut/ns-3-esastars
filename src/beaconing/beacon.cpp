...
namespace ns3 {
    void Beacon::ExtractPathSegmentFromPushBasedBeacon(PathSegment &pathSegment) const {
        ...
        bool last_hop = true;
        for (std::vector<uint64_t>::const_reverse_iterator hop = the_path.rbegin(); hop != the_path.rend(); ++hop) {
            uint16_t ingress = 0;
            uint16_t egress = 0;
            uint16_t as = 0;
    ...
    void Beacon::ExtractPathSegmentFromPullBasedBeacon(PathSegment &pathSegment) const {
        pathSegment.initiation_time = next_initiation_time;
        pathSegment.expiration_time = next_expiration_time;

        pathSegment.originator = (((uint32_t) the_isd_path.back()) << 16) | (((uint32_t) SECOND_LOWER_16_BITS(the_path.back())));
