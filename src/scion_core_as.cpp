...
namespace ns3 {
    ...
    SCION_AS::ScheduleBeaconing (ns3::Time beaconing_period, ns3::Time last_beaconing_event_time) {
        for (Time t = Seconds(0); t < last_beaconing_event_time; t += beaconing_period) {
            ...
            Simulator::Schedule(t + local_time, &BeaconServer::DisseminateBeacons, this->beaconServer, neighbour_relation::CORE);

            Simulator::Schedule(t + local_time, &BeaconServer::InitiateBeacons, this->beaconServer, neighbour_relation::CORE);
            Simulator::Schedule(t + local_time, &BeaconServer::InitiateBeacons, this->beaconServer, neighbour_relation::CUSTOMER);
